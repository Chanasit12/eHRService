...
import th.co.techberry.controller.LeaveCtrl;
            ...
            if(!DayName.equals("SATURDAY") && !DayName.equals("SUNDAY") && ctrl.Check_Holliday(current_date_time[0])){
                ...
                for(Map<String, Object> temp : data){
                    ...
                    if(Leave == null){
                        ...
                        dbutil.Add_Checkin_Checkout_Detail_log(connection,check_model.getCheckId(),Time);
                        Log_detail = dbutil.select2con(connection,"checkin_checkout_detail_log",
                                "Check_id","Time",Integer.toString(check_model.getCheckId()),Time);
                        dbutil.Addlog(connection,"checkin_checkout_log","Check_id",
                                Integer.toString(check_model.getCheckId()),Time,"0","1","Add",(Integer)Log_detail.get("Log_id"));
                    }
                }
            }
